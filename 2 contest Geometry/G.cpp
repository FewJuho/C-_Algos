...
int main() {
  ...
  for (auto i = 0; i < n - 1; i++) {
    vector[i] = geometry::Vector(polygon[i], polygon[i + 1]);
  }
  vector[n - 1] = geometry::Vector(polygon[n - 1], polygon[0]);
  bool check1 = false, check2 = false;
  for (auto i = 1; i < n; i++) {
    double product;
    product = geometry::VectorProduct(vector[i - 1], vector[i]);
    if (product > 0) {
      check1 = true;
    } else if (product < 0) {
      check2 = true;
  ...
  if (geometry::VectorProduct(vector[n - 1], vector[0]) > 0) {
    check1 = true;
  } else if (geometry::VectorProduct(vector[n - 1], vector[0]) < 0) {
    check2 = true;
  }
  if (!(check1 && check2)) {
    std::cout << "YES";
