...
bool Segment::ContainsPoint(const Point&) const {
  return false;
}
...
int main() {
  ...
  geometry::Line first(a1, b1, c1);
  geometry::Line second(a2, b2, c2);

  std::cout << std::fixed << std::setprecision(9) << first.vector_.x_ << ' ' << first.vector_.y_ << std::endl;
  std::cout << std::fixed << std::setprecision(9) << second.vector_.x_ << ' ' << second.vector_.y_ << std::endl;

  if (geometry::VectorProduct(first.vector_, second.vector_) == 0) {
    std::cout << std::fixed << std::setprecision(9) << first.DistToPoint(second.point_);
  } else {
    geometry::Point point_intersect = geometry::CramerRule(a1, b1, c1, a2, b2, c2);
    std::cout << std::fixed << std::setprecision(9) << point_intersect.x_ << ' ' << point_intersect.y_;
  }
}
