...
void ConvexHull(const Polygon& polygon) {
  auto points_polygon = polygon.n_points_;
  Point start_point = points_polygon[0];
  int64_t start_pos = 0;
  for (int i = 0; i < polygon.size_; ++i) {
    if (points_polygon[i] < start_point) {
      start_pos = i;
      start_point = points_polygon[i];
    }
  }
  geometry::Point::x_start = start_point.x_;
  geometry::Point::y_start = start_point.y_;
  std::swap(points_polygon[0], points_polygon[start_pos]);
  std::sort(points_polygon.begin() + 1, points_polygon.end(), Cmp);
  int64_t pos = 1;
  for (size_t j = 1; j < points_polygon.size(); ++j) {
    while (j < points_polygon.size() - 1 && VectorProduct(start_point, points_polygon[j], points_polygon[j + 1]) == 0) {
      ++j;
    }
    points_polygon[pos] = points_polygon[j];
    ++pos;
  }
  points_polygon.resize(pos);
  std::vector<Point> hull;
  hull.reserve(pos);
  hull.push_back(points_polygon[0]);
  hull.push_back(points_polygon[1]);
  hull.push_back(points_polygon[2]);
  for (int i = 3; i < pos; ++i) {
    while (VectorProduct(hull[hull.size() - 2], hull[hull.size() - 1], points_polygon[i]) > 0) {
      hull.pop_back();
    }
    hull.push_back(points_polygon[i]);
  }
  std::vector<Point> new_hull;
  new_hull.reserve(hull.size());
  for (size_t i = 0; i < hull.size(); ++i) {
    if (i == 0) {
      if (VectorProduct(Vector{hull[hull.size() - 1], hull[0]}, Vector{hull[0], hull[1]}) != 0) {
        new_hull.push_back(hull[i]);
      }
    } else if (i == hull.size() - 1) {
      if (VectorProduct(Vector{hull[hull.size() - 2], hull[i]}, Vector{hull[i], hull[0]}) != 0) {
        new_hull.push_back(hull[i]);
      }
    } else {
      if (VectorProduct(Vector{hull[i - 1], hull[i]}, Vector{hull[i], hull[i + 1]}) != 0) {
        new_hull.push_back(hull[i]);
      }
    }
  }
  ...
  for (size_t i = 0; i < new_hull.size(); ++i) {
    std::cout << new_hull[i].x_ << ' ' << new_hull[i].y_ << "\n";
  }
  int64_t s = std::abs(geometry::S(new_hull));
  if (s % 2 == 0) {
    std::cout << s / 2 << ".0";
  } else {
    std::cout << s / 2 << ".5";
