...
Segment::Segment(const double& begin_x, const double& begin_y, const double& end_x, const double& end_y)
...
double S(std::vector<Point> shape) {
  double result = 0;
...
int main() {
  ...
  std::cout << std::fixed << std::setprecision(15) << std::abs(geometry::S(polygon)) / 2.0;
}
